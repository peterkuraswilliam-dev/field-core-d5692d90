          ...
          return (
            ...
            >
              ...
              {update.photos.length > 0 && (
                <div className="mt-3 flex gap-2 overflow-hidden">
                  {update.photos.slice(0, 4).map((photo) => (
                    <div
                      key={photo.id}
                      className="h-14 w-16 shrink-0 overflow-hidden rounded-lg border border-border"
                    >
                      {photo.signed_url && (
              ...
              <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {update.tasks.length} completed task{update.tasks.length === 1 ? "" : "s"}
                </span>
    ...
    <Sheet title="Add Progress Update" onClose={onClose}>
      ...
      <section>
        ...
        <div className="mt-2 flex gap-2 overflow-x-auto pb-1">
          {photos.slice(0, 12).map((photo) => {
            ...
            return (
              ...
              >
                ...
                {selected && (
                  <span className="absolute right-1 top-1 rounded-full bg-gold p-1 text-gold-foreground">
          ...
          {photos.length === 0 && (
            <span className="text-xs text-muted-foreground">No project photos yet.</span>
          )}
        </div>
      </section>
        ...
        <div className="mt-2 space-y-2">
          {tasks.map((task) => {
            ...
            return (
              ...
              >
                <span
                  className={`flex h-5 w-5 items-center justify-center rounded border ${selected ? "border-gold bg-gold text-gold-foreground" : "border-border"}`}
                >
                  {selected && "✓"}
                </span>
    ...
    <Sheet title="Progress Update" onClose={onClose}>
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1">
          {locked && <Lock size={12} />}
          {locked ? "Locked" : "Editable for 24 hours"}
        </span>
      </div>
      <div>
        <Label>Submitted by</Label>
        <p className="mt-1 text-sm text-foreground">
          {update.author_name ?? "Team member"} · {new Date(update.created_at).toLocaleString()}
        </p>
      </div>
      {editing ? (
        <>
        ...
        <>
          ...
          {update.issues && (
            <div>
              <Label>Issues or delays</Label>
              <p className="mt-2 whitespace-pre-line text-sm text-amber-200">{update.issues}</p>
            </div>
          )}
        </>
      )}
      ...
      {mayEdit && !editing && (
        <button
          ...
          className="h-11 w-full rounded-xl border border-gold/40 bg-gold/10 text-sm font-semibold text-gold"
        >
          Edit recent update
        </button>
      )}
...
function Label({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-[10px] uppercase tracking-widest text-muted-foreground">{children}</div>
  );
}
